...
func AuthMiddleware() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if response, err := client.JwtService(ctx, req); err == nil {
			ctx.Request.Header.Set("user", response.GetId())
			ctx.Request.Header.Set("type", response.GetType())
			ctx.Request.Header.Set("email", response.GetEmail())
			ctx.Request.Header.Set("firstname", response.GetFirstname())
			ctx.Request.Header.Set("lastname", response.GetLastname())
			ctx.Request.Header.Set("company", response.GetCompany())
		} else {
